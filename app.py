...
def display_deep_dive_intel(data: Dict[str, Any]):
    ...
    key_map = {"total_addressable_market": "🎯 Total Addressable Market", "competitive_landscape": "⚔️ Competitive Landscape", "moat_analysis": "🏰 Moat Analysis", "valuation_trends": "💹 Valuation Trends", "regulatory_outlook": "📜 Regulatory Outlook", "supply_demand_dynamics": "⚖️ Supply vs Demand"}
    for key, title in key_map.items():
        if key in data:
            st.markdown(f"<h3>{title}</h3>", unsafe_allow_html=True); content = data[key]
...
def render_input_area():
    with card():
        col1, col2 = st.columns([1, 5]); col1.image("Anthill Logo-Falcon.png", width=120); col2.markdown("<h1>Anthill AI+ Evaluation</h1>", unsafe_allow_html=True)
    st.markdown("---"); inputs = {}
    focus_area_map = {"Enhance Urban Lifestyle": ['E-commerce & D2C', 'Consumer Services', 'FinTech', 'PropTech', 'Logistics & Supply Chain', 'Travel & Hospitality', 'Media & Entertainment', 'Gaming'], "Live Healthy": ['Digital Health', 'MedTech', 'BioTech'], "Mitigate Climate Change": ['Clean Energy', 'EV Mobility', 'AgriTech']}
    ...
    with tab1:
        ...
        with card():
            ...
            with c1:
                inputs['company_name'] = st.text_input("Company Name", "InnoTech Bharat"); inputs['focus_area'] = st.selectbox("Investment Focus Area", list(focus_area_map.keys())); inputs['sector'] = st.selectbox("Sector", focus_area_map[inputs['focus_area']]); inputs['location'] = st.selectbox("Location", ["India", "US", "SEA", "MENA", "EU"], index=0)
            with c2:
                inputs['stage'] = st.selectbox("Funding Stage", ["Pre-Seed", "Seed", "Series A", "Series B"], index=2); inputs['founder_type'] = st.selectbox("Founder Archetype", [p.value for p in FounderPersonality])
        with card():
    ...
    with tab2:
        ...
        with card():
            c1, c2, c3 = st.columns(3)
            inputs['arr'] = c1.number_input("Current ARR (₹)", value=80000000, min_value=0); inputs['burn'] = c2.number_input("Monthly Burn (₹)", value=10000000, min_value=0); inputs['cash'] = c3.number_input("Cash Reserves (₹)", value=90000000, min_value=0)
    ...
    st.markdown("---")
    with card():
        c1, c2 = st.columns([4, 1.4]); comps_ticker = c1.text_input("Public Comps Ticker", "ZOMATO.BSE")
    ...
    with card():
        ...
        with col2:
            ...
            with st.container(border=True):
                st.markdown(f"<small>Momentum: <b class='mono'>{ssq.get('momentum', 0)}</b></small>", unsafe_allow_html=True); st.progress(ssq.get('momentum', 0) / 10.0)
                st.markdown(f"<small>Efficiency: <b class='mono'>{ssq.get('efficiency', 0)}</b></small>", unsafe_allow_html=True); st.progress(ssq.get('efficiency', 0) / 10.0)
                st.markdown(f"<small>Scalability: <b class='mono'>{ssq.get('scalability', 0)}</b></small>", unsafe_allow_html=True); st.progress(ssq.get('scalability', 0) / 10.0)
        with col3:
...
def render_analysis_area(report):
    ...
    if c2.button("⬅️ New Analysis"):
        ...
        st.rerun()
    render_summary_dashboard(report); st.markdown("---"); section_heading("Deep Dive Analysis")
    ...
    with tab4:
        with card(): st.json(st.session_state.get('inputs', {}))
