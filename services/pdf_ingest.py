...
class PDFIngestor:
    ...
    def extract_text(self, file_bytes: bytes, max_pages: int = 30) -> str:
    ...
    def _fallback_extract(self, text: str) -> Dict[str, Any]:
        ...
        company = find([r"Company Name[:\-]\s*(.+)", r"Company[:\-]\s*(.+)"]) or "Untitled Company"
        sector = find([r"Sector[:\-]\s*(.+)"]) or "E-commerce & D2C"
        stage = find([r"(Pre-Seed|Seed|Series A|Series B)"], "Series A") or "Series A"
        location = find([r"Location[:\-]\s*(.+)"]) or "India"

        arr = self._coerce_float(find([r"ARR[:\-]\s*₹?([0-9,\.]+)"]))
        burn = self._coerce_float(find([r"Burn[:\-]\s*₹?([0-9,\.]+)"]))
        cash = self._coerce_float(find([r"Cash[:\-]\s*₹?([0-9,\.]+)"]))
        team_size = self._coerce_int(find([r"Team Size[:\-]\s*([0-9,\.]+)"]), 40)

        ltv_cac = self._coerce_float(find([r"LTV\s*\:\s*CAC(?:\s*Ratio)?[:\-]?\s*([0-9\.]+)"]), 3.0)
        gross_margin = self._coerce_float(find([r"Gross Margin(?: %| Percent)?[:\-]?\s*([0-9\.]+)"]), 60.0)
        churn = self._coerce_float(find([r"(?:Monthly )?Churn(?: %| Percent)?[:\-]?\s*([0-9\.]+)"]), 2.0)
        ...
        data = {
            ...
            "arr": max(0.0, arr) or 80_000_000.0,
            "burn": max(0.0, burn) or 10_000_000.0,
            "cash": max(0.0, cash) or 90_000_000.0,
    ...
    def extract(self, file_bytes: bytes) -> Dict[str, Any]:
        ...
        data = self._llm_extract(text) or self._fallback_extract(text)
